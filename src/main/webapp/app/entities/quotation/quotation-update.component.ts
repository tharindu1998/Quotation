...
export class QuotationUpdateComponent implements OnInit {
    _quotation: IQuotation;
    isSaving = false;
    moment: Moment;
    @ViewChild('editForm') form;
    inverters: any | IInverters[];
    inverterBrands = [];
    filteredInverterModels = [];
    inverterFilledWithData = false;

    panelsFilledWithData = false;
    panels: any | IPanels[];
    panelBrands = [];
    filteredPanelModels = [];
    ...
    ngOnInit() {
        ...
        if (!this.quotation.id) {
            this.quotation.clientId =
                this.activatedRoute.snapshot && this.activatedRoute.snapshot.params['clientId']
                    ? this.activatedRoute.snapshot.params['clientId']
                    : '';

            this.quotation.addressId =
                this.activatedRoute.snapshot && this.activatedRoute.snapshot.params['addressId']
        ...
        this.invertersService.query().subscribe(
            (res: HttpResponse<IInverters[]>) => {
                ...
                this.inverterBrands = this.inverterBrands.filter(
                    (value, index, array) => !array.filter((v, i) => JSON.stringify(value) === JSON.stringify(v) && i < index).length
                );
        ...
        this.panelsService.query().subscribe(
            (res: HttpResponse<IPanels[]>) => {
                ...
                this.panelBrands = this.panelBrands.filter(
                    (value, index, array) => !array.filter((v, i) => JSON.stringify(value) === JSON.stringify(v) && i < index).length
                );
        ...
        this.form.valueChanges.subscribe(res => {
            ...
            this._quotation.unitsGenerates = Number(
                parseFloat(String(this._quotation.numberofPanels * (30 * 3.8 * (this._quotation.panelCapacity / 1000)))).toFixed(2)
            );
        });
    }
    ...
    systemSizeChanged(obj) {
        this._quotation.numberofPanels = Number(
            parseFloat(String(this._quotation.systemSize * 1000 / this._quotation.panelCapacity)).toFixed(0)
        );
    }
