...
export class Page3Component implements OnInit {
    ...
    ngOnInit() {
        this.systemSize =
            this.quotation.inverterCapacity * this.quotation.numberOfInverters +
            this.quotation.inverterCapacity2 * this.quotation.numberOfInverters2;
        this.panelCap = this.quotation.panelModel.split(',')[1];

        if (this.quotation.inverterCapacity2) {
            this.inv =
                this.quotation.inverterCapacity +
                ' * ' +
                this.quotation.numberOfInverters +
                ' + ' +
                this.quotation.inverterCapacity2 +
                ' * ' +
                this.quotation.numberOfInverters2;
        } else {
